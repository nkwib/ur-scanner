...
A third clock used to make this worse: the receiver's own fixed scan timer. Sampling a changing signal on an unrelated fixed interval aliases against it, so some parts got decoded twice and others were never looked at. Scanning per delivered camera frame removes that clock, which is why the measured gain from removing it tracks sender fps: nothing at 6 fps, where the sender is the limit, and about 7x at 30 fps. The gain is not smooth in between (a 10 fps sender measured slower than a 15 fps one under the old timer), and that unevenness *is* the aliasing, not noise in the benchmark.
